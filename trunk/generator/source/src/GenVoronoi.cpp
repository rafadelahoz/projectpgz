...
struct Halfedge* GenVoronoi::HEcreate(struct Edge *e,int pm)
{
...
void GenVoronoi::ELinsert(struct	Halfedge *lb, struct Halfedge *newHe)
{
	newHe -> ELleft = lb;
...
struct Halfedge * GenVoronoi::ELleftbnd(struct PointVDG *p)
{
	int i, bucket;
	struct Halfedge *he;
...
void GenVoronoi::ELdelete(struct Halfedge *he)
{
...
struct Halfedge * GenVoronoi::ELright(struct Halfedge *he)
{
	return (he -> ELright);
}

struct Halfedge * GenVoronoi::ELleft(struct Halfedge *he)
{
	return (he -> ELleft);
}


struct Site * GenVoronoi::leftreg(struct Halfedge *he)
{
...
struct Site * GenVoronoi::rightreg(struct Halfedge *he)
{
...
struct Edge * GenVoronoi::bisect(struct Site *s1,struct Site *s2)
{
	float dx,dy,adx,ady;
	struct Edge *newedge;
...
struct Site * GenVoronoi::intersect(struct Halfedge *el1, struct Halfedge *el2, struct PointVDG *p)
{
	struct	Edge *e1,*e2, *e;
	struct  Halfedge *el;
	float d, xint, yint;
...
int GenVoronoi::right_of(struct Halfedge *el,struct PointVDG *p)
{
	struct Edge *e;
	struct Site *topsite;
...
void GenVoronoi::endpoint(struct Edge *e,int lr,struct Site * s)
{
	e -> ep[lr] = s;
	ref(s);
...
float GenVoronoi::dist(struct Site *s,struct Site *t)
{
	float dx,dy;
...
void GenVoronoi::makevertex(struct Site *v)
{
	v -> sitenbr = nvertices;
...
void GenVoronoi::deref(struct Site *v)
{
	v -> refcnt -= 1;
	if (v -> refcnt == 0 ) 
...
void GenVoronoi::ref(struct Site *v)
{
	v -> refcnt += 1;
	v -> overallRefcnt += 1;
}
...
void GenVoronoi::PQinsert(struct Halfedge *he,struct Site * v, float offset)
{
...
void GenVoronoi::PQdelete(struct Halfedge *he)
{
...
int GenVoronoi::PQbucket(struct Halfedge *he)
{
	int bucket;
...
int GenVoronoi::PQempty()
{
	return(PQcount==0);
}


struct PointVDG GenVoronoi::PQ_min()
{
...
struct Halfedge * GenVoronoi::PQextractmin()
{
...
void GenVoronoi::freeinit(struct Freelist *fl,int size)
{
	fl -> head = (struct Freenode *) NULL;
	fl -> nodesize = size;
}

char * GenVoronoi::getfree(struct Freelist *fl)
{
	int i; 
	struct Freenode *t;
...
void GenVoronoi::makefree(struct Freenode *curr,struct Freelist *fl)
{
	curr -> nextfree = fl -> head;
	fl -> head = curr;
}

void GenVoronoi::cleanup()
{
	if(sites != 0)
	{
		free(sites);
...
/* #include <plot.h> */
void GenVoronoi::openpl(){}
void GenVoronoi::line(float x1, float y1, float x2, float y2)
{	
...
void GenVoronoi::clip_line(struct Edge *e)
{
	struct Site *s1, *s2;
...
bool GenVoronoi::voronoi(bool genVertexInfo)
{
...
void GenVoronoi::insertVertexAddress(long vertexNum, struct Site* address)
{
	if(vertices == 0)
	{
...
void GenVoronoi::insertVertexLink(long vertexNum, long vertexLinkedTo)
{
	if(vertexLinks == 0)
	{
...
void GenVoronoi::generateVertexLinks()
{
	long i = 0, j = 0;	
	if(finalVertexLinks != 0)
	{
...
bool GenVoronoi::getNextVertexPair(float& x1, float& y1, float& x2, float& y2)
{
	if(finalVertexLinks == 0)
		return false;
...
struct Site * GenVoronoi::nextone()
{
	struct Site *s;
	if(siteidx < nsites)
	{	
